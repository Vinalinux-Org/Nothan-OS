...
- `nothan-kernel/build/kernel.bin` — kernel với userspace ELFs nhúng sẵn
- `userspace/<app>/<app>.elf` — init, shell và các utilities
...
│   ├── sbin/init       PID 1: spawn shell, respawn, reap zombie
│   ├── bin/sh          shell: pipe, redirect, background job, builtins
│   ├── bin/{ls,ps,uname,info,reboot,shutdown}
│   └── lib/            nothanlibc (POSIX subset)
│
