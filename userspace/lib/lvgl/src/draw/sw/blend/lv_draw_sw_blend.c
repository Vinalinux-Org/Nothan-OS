...
void lv_draw_sw_blend(lv_draw_task_t * t, const lv_draw_sw_blend_dsc_t * blend_dsc)
{
    ...
    if(blend_dsc->src_buf == NULL) {
        ...
        /*
         * NothanOS guard (debug): the hardware Data Abort (DFAR=0x2d6000, in
         * lv_draw_sw_blend_color_to_rgb565 while scrolling) is a fill whose
         * dest_w is garbage-huge — the per-row loop walks the mask/dest
         * pointer hundreds of KB past the small pool buffer into the first
         * unmapped page above __bss_end. A draw task's blend area can never
         * exceed the layer's render area, so a dest_w/h beyond it is corrupt:
         * log the offending geometry (to trace the source) and skip the blend
         * so the GUI survives instead of dying. Remove once root-caused.
         */
        {
            int32_t buf_w = lv_area_get_width(&layer->buf_area);
            int32_t buf_h = lv_area_get_height(&layer->buf_area);
            if(fill_dsc.dest_w < 0 || fill_dsc.dest_h < 0 ||
               fill_dsc.dest_w > buf_w || fill_dsc.dest_h > buf_h) {
                LV_LOG_ERROR("CORRUPT fill area: dest=%dx%d buf_area=%dx%d "
                             "blend(orig)=(%d,%d %d,%d) clip=(%d,%d %d,%d) "
                             "isect=(%d,%d %d,%d)",
                             (int)fill_dsc.dest_w, (int)fill_dsc.dest_h,
                             (int)buf_w, (int)buf_h,
                             (int)blend_dsc->blend_area->x1, (int)blend_dsc->blend_area->y1,
                             (int)blend_dsc->blend_area->x2, (int)blend_dsc->blend_area->y2,
                             (int)t->clip_area.x1, (int)t->clip_area.y1,
                             (int)t->clip_area.x2, (int)t->clip_area.y2,
