/*
 * arch/arm/kernel/exception_handlers.c
 *
 * C-level handlers for ARM hardware exceptions.  Two outcomes are possible:
 *
 *   - Exception in user mode (SPSR[4:0] == 0x10): deliver SIGSEGV by calling
 *     do_exit() with exit code 139.  The task is terminated; the kernel
 *     continues running normally.
 *
 *   - Exception in any privileged mode: no safe recovery is possible; print
 *     diagnostic information and spin in halt_kernel().
 *
 * FAULT_EXIT_SIGSEGV is 128 + 11 (SIGSEGV signal number), matching the
 * exit-status convention used by shells and wait() callers.
 */
...
void handle_prefetch_abort(struct exception_context *ctx)
{
    ...
    if ((ctx->spsr & 0x1F) == 0x10)
    {
        ...
        /*
         * Switch back to SVC mode before calling do_exit().  The prefetch
         * abort was taken in ABT mode; C kernel paths expect SVC mode.
         */
...
/*
 * decode_fault_status - decode the Fault Status field from DFSR
 *
 * In the ARMv7-A short-descriptor translation table format, the fault
 * status is a 5-bit field: bit[10] of DFSR concatenated with bits[3:0].
 * The table below maps these 5-bit codes to human-readable strings.
 */
static const char *decode_fault_status(uint32_t dfsr)
{
    ...
    case 0x0C: return "Sync external abort on translation (L1)";
    case 0x0D: return "Permission fault (Section)";
    case 0x0E: return "Sync external abort on translation (L2)";
