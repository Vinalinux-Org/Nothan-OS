/*
 * bootloader/src/mmc.c - AM335x MMC0 raw-sector read driver
 *
...
#define MMC_CMD_CCCE		BIT(19)	/* CRC check enable */
#define MMC_CMD_CICE		BIT(20)	/* index check enable */
...
static int mmc_send_cmd(uint32_t cmd, uint32_t arg, uint32_t flags)
{
	...
	if (status & MMC_STAT_ERRI) {
		/*
		 * SD spec: must reset cmd line after error or the controller
		 * stays wedged and ignores subsequent commands.
		 */
...
int mmc_init(void)
{
	...
	/* Keep clocks active during init — prevents PM from gating mid-sequence. */
	...
	/*
	 * SD spec: set bus voltage before enabling bus power. Reversing the
	 * order leaves the card in an undefined state.
	 */
	...
	while ((readl(MMC_HCTL) & MMC_HCTL_SDBP) == 0) {
		if (--timeout == 0) {
			uart_puts("MMC:    Failed to enable bus power\r\n");
			return -1;
		}
	}
	...
	while ((readl(MMC_SYSCTL) & MMC_SYSCTL_ICS) == 0) {
		if (--timeout == 0) {
			uart_puts("MMC:    Timeout waiting for clock stable\r\n");
			return -1;
		}
	}
	...
	/*
	 * Polled mode still requires IE/ISE — controller only sets STAT bits
	 * when the corresponding IE bit is set.
	 */
	writel(0xFFFFFFFF, MMC_IE);
	writel(0xFFFFFFFF, MMC_ISE);
	writel(0xFFFFFFFF, MMC_STAT);

	/* SD spec: drive >= 80 clocks before sending any command. */
	...
	for (int i = 0; i < 2000; i++) {
		if (mmc_send_cmd(55, 0, MMC_RSP_R1) != 0)
			continue;
	...
	/*
	 * Switch to ~24 MHz: disable CEN → reprogram CLKD → wait ICS →
	 * re-enable CEN. Order matters — changing CLKD while CEN=1 is
	 * undefined per OMAP TRM.
	 */
	{
		uint32_t sysctl;

		sysctl = readl(MMC_SYSCTL) & ~(1 << 2);
		writel(sysctl, MMC_SYSCTL);

		sysctl = (readl(MMC_SYSCTL) & ~0xFFC0) | (4 << 6);
		writel(sysctl, MMC_SYSCTL);
	...
	mmc_send_cmd(16, 512, MMC_RSP_R1);	/* CMD16: fix block length for SDSC */
	return 0;
}
...
int mmc_read_sectors(uint32_t start_sector, uint32_t count, void *dest)
{
	...
	for (i = 0; i < count; i++) {
		...
		/* SDHC uses block number; SDSC uses byte address */
		uint32_t arg = sdhc_card ? (start_sector + i)
					 : ((start_sector + i) * 512);
		writel(arg, MMC_ARG);
		writel((17 << 24) | MMC_RSP_R1 | MMC_CMD_DP | MMC_CMD_DDIR_READ,
		       MMC_CMD);

		int timeout = 10000000;
		while ((readl(MMC_STAT) & MMC_STAT_BRR) == 0) {
			if (readl(MMC_STAT) & MMC_STAT_ERRI) {
				panic("MMC Read ERR");
				return -1;
			}
			if (--timeout == 0) {
