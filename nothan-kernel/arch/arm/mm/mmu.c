...
void mmu_init(void)
{
    ...
    pr_info("[MMU] Peripheral USB:     PA 0x%x (%d MB) [Strongly Ordered, Identity]\n",
                PERIPH_USB_PA, PERIPH_USB_SECTIONS);
...
uint32_t mmu_new_pgd(void)
{
    ...
    for (uint32_t i = 0; i < PERIPH_USB_SECTIONS; i++)
    {
        uint32_t idx = (PERIPH_USB_PA >> MMU_SECTION_SHIFT) + i;
        new_pgd[idx] = pgd[idx];
    }
...
mmu_build_page_table_boot(uint32_t *pgd_pa)
{
    ...
    /* USB Subsystem (1MB) */
    for (i = 0; i < PERIPH_USB_SECTIONS; i++)
    {
        pa = PERIPH_USB_PA + (i * MMU_SECTION_SIZE);
        pgd_pa[pa >> MMU_SECTION_SHIFT] = pa | MMU_SECT_PERIPHERAL;
    }
