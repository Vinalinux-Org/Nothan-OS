...
/**
 * mock_bdev_init() - Initialize the in-memory mock block device
 *
 * Builds a fake FAT32 volume in a static RAM buffer so the VFS and FAT32
 * driver can be exercised without real SD card hardware. Layout:
 *   Sector  0:      BPB (512B/sector, 8s/cluster, 32 reserved, 2 FATs)
 *   Sector 32:      FAT table (clusters 2-7, all EOC)
 *   Sector 64:      Root directory (EXAMPLE.BIN, /bin, /sbin)
 *   Sector 72:      SHELL.BIN data (deleted in root dir)
 *   Sector 80:      /bin directory (hello.bin → cluster 6)
 *   Sector 88:      /sbin directory (empty)
 *   Sector 96:      /bin/hello.bin data
 *   Sector 104:     EXAMPLE.BIN binary (74 bytes, ARM Thumb code)
 */
void mock_bdev_init(void)
{
	/* BPB fields — byte offsets per FAT32 spec (little-endian). */
	u8 *bpb = mock_ram_disk;
	bpb[11] = 0x00; bpb[12] = 0x02;                /* bytes per sector: 512 */
	bpb[13] = 8;                                    /* sectors per cluster: 8 (4 KB) */
	bpb[14] = 32;   bpb[15] = 0;                   /* reserved sectors: 32 */
	bpb[16] = 2;                                    /* number of FATs: 2 */
	bpb[32] = 0x00; bpb[33] = 0x20; bpb[34] = 0x00; bpb[35] = 0x00; /* total sectors: 8192 */
	...
	/* Boot sector signature — required by FAT spec, checked on mount. */
	bpb[510] = 0x55;
	bpb[511] = 0xAA;

	/* FAT table: mark clusters 2-7 as end-of-chain (EOC = 0x0FFFFFFF). */
	...
	/* Entry 0: previously SHELL.BIN, now deleted.
	 * 0xE5 in name[0] is the FAT32 deleted-entry marker. */
	root_dir[0] = 0xE5;

	/* Entry 1: EXAMPLE.BIN (archive, cluster 7, 74 bytes). */
	root_dir[32] = 'E'; root_dir[33] = 'X'; root_dir[34] = 'A';
	root_dir[35] = 'M'; root_dir[36] = 'P'; root_dir[37] = 'L';
	root_dir[38] = 'E'; root_dir[39] = ' ';
	root_dir[40] = 'B'; root_dir[41] = 'I'; root_dir[42] = 'N';
	root_dir[43] = 0x20;                    /* attr: archive */
	root_dir[52] = 0;   root_dir[53] = 0;   /* fst_clus_hi: 0 */
	root_dir[58] = 7;   root_dir[59] = 0;   /* fst_clus_lo: 7 */
	root_dir[60] = 74;  root_dir[61] = 0; root_dir[62] = 0; root_dir[63] = 0;

	/* Entry 2: /bin directory (cluster 4). */
	int e2 = 64;
	root_dir[e2+0]  = 'B'; root_dir[e2+1] = 'I'; root_dir[e2+2] = 'N';
	root_dir[e2+11] = 0x10;                 /* attr: directory */
	root_dir[e2+20] = 0;   root_dir[e2+21] = 0;
	root_dir[e2+26] = 4;   root_dir[e2+27] = 0;

	/* Entry 3: /sbin directory (cluster 5). */
	int e3 = 96;
	root_dir[e3+0]  = 'S'; root_dir[e3+1] = 'B';
	root_dir[e3+2]  = 'I'; root_dir[e3+3] = 'N';
	root_dir[e3+11] = 0x10;                 /* attr: directory */
	root_dir[e3+20] = 0;   root_dir[e3+21] = 0;
	root_dir[e3+26] = 5;   root_dir[e3+27] = 0;

	/* Entry 4: implicit end-of-directory (RAM is zero-initialised). */

	/* SHELL.BIN payload at cluster 3 (sector 72) — kept for read tests. */
	u8 *shell_data = mock_ram_disk + (72 * 512);
	const char *msg = "Hello from NothanOS FAT32 mock file system!";
	for (int i = 0; msg[i]; i++)
		shell_data[i] = msg[i];

	/* /bin directory at cluster 4 (sector 80): single entry hello.bin. */
	u8 *bin_dir = mock_ram_disk + (80 * 512);
	bin_dir[0] = 'H'; bin_dir[1] = 'E'; bin_dir[2] = 'L';
	bin_dir[3] = 'L'; bin_dir[4] = 'O';
	bin_dir[5] = ' '; bin_dir[6] = ' '; bin_dir[7] = ' ';
	bin_dir[8] = 'B'; bin_dir[9] = 'I'; bin_dir[10] = 'N';
	bin_dir[11] = 0x20;                     /* attr: archive */
	bin_dir[20] = 0;  bin_dir[21] = 0;
	bin_dir[26] = 6;  bin_dir[27] = 0;      /* fst_clus_lo: 6 */
	...
	for (int i = 0; hmsg[i]; i++)
		hello_data[i] = hmsg[i];

	/* /sbin at cluster 5 (sector 88): empty directory, no entries needed. */

	/* EXAMPLE.BIN at cluster 7 (sector 104): 74-byte ARM binary. */
	...
	printk("[BLOCK] Mock RAM disk initialized\n");
}
